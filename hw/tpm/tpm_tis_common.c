...
#define DEBUG_TIS 0
#define HDATA_BUFFER_SIZE 4096
uint8_t hash_started = 0;
uint8_t hdata[HDATA_BUFFER_SIZE] = {0};
int pending_data_size = 0;
                               ...
                               uint64_t val, unsigned size)
{
    ...
    switch (off) {
    case TPM_TIS_HASH_START:
        /* Hashing operations only permitted on locality 4 */
        if (locty == 4){
            /*Locality has to be 4 or None to consider a HASH_START command*/
                tpm_backend_hash_start(s->be_driver);
                hash_started = 1;
                break;
                }
        break;
    case TPM_TIS_HASH_END:
        /* Hashing operations only permitted on locality 4 */
        if (locty == 4){
            /*Locality has to be 4 or None to consider a HASH_START command*/
                if (active_locty != 4 && active_locty != TPM_TIS_NO_LOCALITY){
                    break;
                }

                if (pending_data_size > 0) {
                    tpm_backend_hash_data(s->be_driver, hdata, HDATA_BUFFER_SIZE);
                }
                tpm_backend_hash_end(s->be_driver);
                hash_started = 0;
                break;
                }
        break;
    ...
    case TPM_TIS_REG_DATA_XFIFO ... TPM_TIS_REG_DATA_XFIFO_END:
        ...
        if (s->loc[locty].state == TPM_TIS_STATE_IDLE ||
            ...
            if (locty == 4){
                if (hash_started) {
                    if (pending_data_size + size < HDATA_BUFFER_SIZE) {
                        memcpy(&(hdata[pending_data_size]), &val, size);
                        pending_data_size += 4;
		    }else{
			tpm_backend_hash_data(s->be_driver, hdata, HDATA_BUFFER_SIZE);
			pending_data_size = 0;
